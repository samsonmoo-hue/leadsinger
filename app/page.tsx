...
export default function Home() {
  ...
  return <main className={room && !editing ? `${role}-screen room-screen` : undefined}>
    {!room && <header><a className="brand" href="./"><span className="brand-icon"><AudioLines /></span><span>領唱提示</span></a><span className={`connection ${connected ? 'online' : ''}`}><i />{connected ? '已連線' : '連線中／重試中'}</span></header>}
    {!room ? <section className="lobby">
      <span className="eyebrow lobby-tagline">LET’S PLAY TOGETHER</span>
      <a className="secondary" href="./guide.html">使用說明</a>
