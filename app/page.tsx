...
import { AudioLines, Mic2, Guitar, ArrowRight, Plus, RotateCcw, Repeat2, Flag, Play, LogOut, Radio, ChevronDown, ChevronLeft, ChevronRight, ListMusic, Trash2, Heart } from 'lucide-react';
...
export default function Home() {
  ...
  return <main className={room && !editing ? `${role}-screen room-screen` : undefined}>
    {!room && <header><a className="brand" href="./"><span className="brand-icon"><AudioLines /></span><span>領唱提示</span></a><div className="header-actions"><span className={`connection ${connected ? 'online' : ''}`}><i />{connected ? '已連線' : '連線中／重試中'}</span><a className="secondary" href="./guide.html">使用說明</a></div></header>}
    {!room ? <section className="lobby">
      ...
      <form onSubmit={enter}><span className="eyebrow">{mode === 'create' ? 'CREATE A ROOM' : 'JOIN THE SESSION'}</span><h2>{mode === 'create' ? '先建立團隊的房間' : '準備好，一起進入'}</h2><p className="form-note">{mode === 'create' ? '可使用聖經人名或自行輸入，下一步確認詩歌與順序。' : '加入領唱的房間，整份歌單都會自動同步。'}</p>
      {mode === 'create' ? <label>房間名稱<div className="room-name-row"><input value={name} onChange={e => setName(e.target.value)} maxLength={80} required disabled={busy} aria-label="房間名稱" /><button type="button" className="secondary" disabled={busy} onClick={() => setName(suggestRoomName([...rooms.map(item => item.name), name]))}><RotateCcw size={17} />換一個</button></div></label> : <><label>選擇房間<span className="song-select"><select value={selected} onChange={e => setSelected(e.target.value)} required><option value="">{rooms.length ? '請選擇房間' : '尚無房間，請領唱先建立'}</option>{rooms.map(r => <option key={r.id} value={r.id}>{r.name}{!r.songs.length ? '（準備歌單中）' : ''}</option>)}</select><ChevronDown size={20} aria-hidden="true" /></span></label><fieldset><legend>選擇身分</legend><div className="roles">{[{ id: 'leader', label: '領唱', sub: '管理歌單與發送提示', Icon: Mic2 }, { id: 'musician', label: '樂手', sub: '觀看同步提示', Icon: Guitar }].map(({ id, label, sub, Icon }) => <label className={`role ${role === id ? 'selected' : ''}`} key={id}><input type="radio" name="role" value={id} checked={role === id} disabled={busy} onChange={() => setRole(id)} /><Icon size={25} /><strong>{label}</strong><small>{sub}</small></label>)}</div></fieldset></>}
      {errorMessage}<button className="primary" disabled={locked || (mode === 'join' ? !selected : !name.trim())}>{busy ? '處理中…' : mode === 'create' ? '建立房間，確認詩歌' : '進入房間'}<ArrowRight size={20} /></button></form><div className="card-footer"><Radio size={16} />加入一次，跟隨領唱同步切歌與提示</div></div>
    ...
    </section> : <section className="stage"><div className="room-heading"><div><span className="eyebrow">{room.name} · {role === 'leader' ? '領唱' : '樂手'}</span><div className="song-title"><h1 aria-live="polite">{currentSong?.name || '領唱準備歌單中'}</h1>{currentSong?.bpm && <span key={`${currentSong.id}-${currentSong.bpm}`} className="song-beat" role="img" aria-label={`每分鐘 ${currentSong.bpm} 拍`} title={`${currentSong.bpm} BPM`}><Heart aria-hidden="true" className="beat-heart" fill="currentColor" style={{ animationDuration: `${60 / currentSong.bpm}s` }} /><span>{currentSong.bpm}</span></span>}</div>{currentSong && <p className="song-position">第 {currentIndex + 1} 首 / 共 {room.songs.length} 首</p>}</div><div className="room-actions">{role === 'leader' && <button className="leave" disabled={locked} onClick={() => editPlaylist(room)}><ListMusic size={18} />詩歌確認</button>}<button className="leave" disabled={busy} onClick={leave}><LogOut size={18} />離開房間</button></div></div>
